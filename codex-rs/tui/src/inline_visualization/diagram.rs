//! Terminal-aware palette preparation for D2 and Mermaid images.
...
pub(super) fn prepare(
    ...
    for pixel in image.pixels_mut() {
        ...
        if source_background
            .is_some_and(|background| color_distance(source_rgb, background) <= BACKGROUND_DISTANCE)
        {
            *pixel = Rgba([0, 0, 0, 0]);
            continue;
        }
        let mapped = map_color(source_rgb, palette, surface, accent_surface);
        *pixel = Rgba([mapped.0, mapped.1, mapped.2, pixel.0[3]]);
    }
