//! Turn-local preflight compilation for assistant-authored visualization fences.
...
pub(crate) fn inline_visualization_compiler_tool_spec() -> DynamicToolSpec {
    DynamicToolSpec::Function(DynamicToolFunctionSpec {
        ...
        description: concat!(
            "Compile one Mermaid, LaTeX, or bounded DOT visualization before emitting it in the ",
            "final answer. Use a stable artifact_id for the initial call and at most two ",
            "syntax-fix retries. After a successful compile, copy the exact source unchanged ",
            "into a top-level fenced block in the final answer."
        )
        .to_string(),
...
impl InlineVisualizationCompileArgs {
    pub(crate) fn parse(arguments: Value) -> Result<Self, String> {
        ...
        if !matches!(arguments.format.as_str(), "mermaid" | "latex" | "dot") {
            return Err("format must be mermaid, latex, or dot".to_string());
        }
...
pub(crate) async fn compile_inline_visualization(
    ...
    match result {
        ...
        Err(error) => {
            ...
            let retryable = retries_remaining > 0 && source_correction_can_help(&diagnostic);
...
fn source_correction_can_help(diagnostic: &str) -> bool {
    ![
        "launch ",
        "timed out",
        "did not create its PNG output",
        "did not produce a bounded PNG",
        "did not produce a valid PNG",
        "cache collision",
        "create inline visualization",
    ]
    .iter()
    .any(|failure| diagnostic.contains(failure))
}
