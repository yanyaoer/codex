...
#[test]
fn preparation_replaces_card_with_terminal_background_and_maps_palette() {
...
#[test]
fn transparent_source_uses_terminal_background_without_card_detection() {
    let temporary = tempfile::tempdir().expect("temp dir");
    let source_path = temporary.path().join("source.png");
    let cache = temporary.path().join("cache");
    ...
    let output = image::open(&prepared.path)
        .expect("decode prepared diagram")
        .into_rgba8();
    assert_eq!(output.get_pixel(0, 0).0, [30, 30, 30, 255]);
    assert!(output.pixels().all(|pixel| pixel.0[3] == 255));
}
